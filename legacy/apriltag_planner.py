...
from geometry_msgs.msg import PoseStamped  # <-- target pose from AprilTag
...
class OdomListener(Node):
    def __init__(self, pose_store: OdomPose):
        super().__init__('odom_and_target_listener')
        self.pose_store = pose_store
        self.have_target = False
        self.dx = 0.0; self.dy = 0.0; self.dth = 0.0
        ...
        # Reuse both, whichever publishes:
        ...
        # AprilTag estimator's target pose (relative in camera/body frame)
    ...
    def target_cb(self, msg: PoseStamped):
        ...
        with self.tlock:
            self.dx, self.dy, self.dth = float(x), float(y), float(yaw)
            self.have_target = True

    def get_target_relative(self):
        with self.tlock:
            return self.dx, self.dy, self.dth, self.have_target
...
def p_go_to(motor: Motor,
    ...
    # phase 1: turn toward goal direction
    while True:
...
def plan_from_apriltag(listener: OdomListener):
    dx, dy, dth, have = listener.get_target_relative()
    if not have:
        raise RuntimeError("No AprilTag /target_pose yet")
    return dx, dy, dth
...
def main():
    ...
    try:
        ...
        while True:
            ...
            try:
                dx, dy, dth = plan_from_apriltag(node)   # <-- from AprilTag
            except RuntimeError:
                continue  # no target yet; keep spinning
